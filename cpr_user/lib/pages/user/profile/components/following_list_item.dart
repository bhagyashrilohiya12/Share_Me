...
class FollowingListItem extends StatefulWidget {
  ...
  FollowingListItem(this.follower,this.helper,this.showProgress);

  @override
  _FollowingListItemState createState() => _FollowingListItemState();
}
...
class _FollowingListItemState extends State<FollowingListItem> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        CPRUser? user = await userService.getUserById(widget.follower.followingId!);
          ...
          children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            widget.follower.following!.username?? "",
                          ...
                          Text(
                            widget.follower.following!.email!.substring(0, 4) +
                                "*******" +
                                widget.follower.following!.email!
                                    .substring(widget.follower.following!.email!.indexOf("@"), widget.follower.following!.email!.length),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                          ),
                        ],
            ...
            Container(
              ...
              child: GestureDetector(
                  onTap: () {
                    if (!widget.helper.followUnfollowProgress! && widget.helper.isYourFollowing != null && loginedUser?.email != widget.follower.following?.email) {
  ...
  follow(BuildContext context) async {
    ...
    var objFollow = new CPRFollowerFollowing(
        follower: provider.user!, followerId: provider.user!.email!, following: widget.follower.following,
        followingId: widget.follower.following!.email!);

    CPRFollowerFollowing? cPRFollowerFollowing = await followingFollowerService.follow(objFollow);
  ...
  _checkIsYourFollowing(BuildContext context) async {
    SessionProvider provider = p.Provider.of<SessionProvider>(context, listen: false);
    FollowingFollowerService followingFollowerService = new FollowingFollowerService();
    CPRFollowerFollowing? cPRFF = await followingFollowerService.isYourFollowing(provider.user!.email!,
        widget.follower.following!.email!);
  ...
  getImageProvider() {
    /**
     #abdo
        widget.follower.following.profilePictureURL != null && widget.follower.following.profilePictureURL.length > 0
        ? NetworkImage(widget.follower.following.profilePictureURL)
        : AssetImage("assets/images/no_avatar_image_choosed.png")
     */

     var link = widget.follower.following!.profilePictureURL!;
    if( ToolsValidation.isValid( link )  ) {
